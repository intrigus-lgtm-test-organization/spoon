...
public abstract class CtReferenceImpl extends CtElementImpl implements CtReference, Serializable {
	...
	private static final Pattern SIMPLE_NAME_REPLACE = Pattern.compile("\\[\\]|@");
	private static final Pattern SIMPLE_NAME_SPLIT = Pattern.compile("\\.|<|>");
	private static final Pattern SIMPLE_NAME_MATCH = Pattern.compile("<.*>|\\d.*|^.{0}$");
	...
	private void checkIdentiferForJLSCorrectness(String simplename) {
		...
		if (!SIMPLE_NAME_MATCH.matcher(simplename).matches()) {
			//split at "<" and ">" because "Iterator<Cache.Entry<K,Store.ValueHolder<V>>>" submits setSimplename ("Cache.Entry<K")
			String[] splittedSimplename = SIMPLE_NAME_SPLIT.split(simplename);
	...
	private boolean checkAllParts(String[] simplenameParts) {
		for (String simpleName:simplenameParts) {
			...
			simpleName = SIMPLE_NAME_REPLACE.matcher(simpleName).replaceAll("");
